...
const AddNewTaskPage = () => {
    ...
    const openNotificationWithIcon = (data: AddNewTask) => {
        const date = new Date(data.taskDate);
        notification["error"]({
            message: 'Wrong hours value for that date',
            description: `Value ${data.hours} hours is incorrect for ${dateFormat(date.getFullYear(), date.getMonth() + 1, date.getDate())}`,
        });
    }
    const onSubmit = (data: AddNewTask) => {
        ...
        else openNotificationWithIcon(data);
